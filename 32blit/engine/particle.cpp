...
#include "engine.hpp"
   ...
   */
  generator::generator(uint32_t count, uint32_t lifetime_ms, std::function<particle*(void)> generate) : count(count), lifetime_ms(lifetime_ms), generate(generate) {    
  }
   ...
   */
  void generator::update(uint32_t time_ms) {
